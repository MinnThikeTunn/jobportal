    ...
    <Combobox store={combobox} onOptionSubmit={handleValueSelect} withinPortal={false}>
      <Combobox.DropdownTarget>
        <PillsInput variant='unstyled' rightSection={<IconSelector />} onClick={() => combobox.toggleDropdown()}
            leftSection={<div className='text-bright-sun-400 p-1 mr-2 bg-mine-shaft-900 rounded-full'><props.icon/></div>}>
          <Pill.Group>
            {value.length > 0 ? (
              <>
                {values}
              ...
              <InputPlaceholder className='!text-mine-shaft-200'>{props.title}</InputPlaceholder>
            )}
