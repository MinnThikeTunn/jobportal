...
const Card=(props:any)=>{
    ...
    return <div className="bg-mine-shaft-900 p-4 w-72 flex flex-col gap-3 rounded-xl hover:shadow-[0_0_5px_1px_yellow] !shadow-bright-sun-400">
            <div className="flex justify-between">
                ...
                {profile.savedJobs?.includes(props.jobId)?<IconBookmarkFilled onClick={handleSaveJob} className=" text-bright-sun-400 cursor-pointer" stroke={1.5}/>:<IconBookmark onClick={handleSaveJob} className="text-mine-shaft-300 hover:text-bright-sun-400 cursor-pointer" stroke={1.5}/>}
            </div>
            <div className="flex gap-2 ">
                <div className="py-2 px-2 bg-mine-shaft-800 text-bright-sun-400 rounded-lg text-xs">{props.experience}</div>
                <div className="py-2 px-2 bg-mine-shaft-800 text-bright-sun-400 rounded-lg text-xs">{props.jobType}</div>
                <div className="py-2 px-2 bg-mine-shaft-800 text-bright-sun-400 rounded-lg text-xs">{props.location}</div>
            </div>
            ...
            {(props.offered || props.interviewing)&&<Divider size="sm" color="#4f4f4f " />}
            {
                props.offered&&<div className="flex gap-2">
                    <Button color="#ffbd20" variant="outline" fullWidth>Accept</Button>
                    <Button color="#ffbd20" variant="light" fullWidth>Reject</Button>
                </div>
            }
            {
            ...
            <Link to={`/jobs/${props.jobId}`}>
                <Button fullWidth color="#ffbd20" variant="outline" >View Job</Button>
            </Link>
        </div>
}
export default Card;